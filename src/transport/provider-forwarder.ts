...
import { apiKindForPath, extractTaskDescription, type OpenAiRequest } from '../model/openai-api.js';
...
export class ProviderForwarder {
  ...
  supports(req: IncomingMessage): boolean {
    return req.method === 'POST' && apiKindForPath(req.url ?? '/') !== null;
  }

  async forward(req: IncomingMessage, res: ServerResponse): Promise<void> {
    const api = apiKindForPath(req.url ?? '/');
    if (!api || req.method !== 'POST') throw new Error('Unsupported provider route');
    ...
    try { body = JSON.parse(raw.toString('utf8')) as OpenAiRequest; }
    ...
    try {
      ...
      try {
        decision = await this.options.releaseController.evaluate({
          api, request: body, taskDescription: extractTaskDescription(api, body),
