...
export class AtpEnforcementController implements ResponseReleaseController {
  ...
  async evaluate(exchange: ModelExchange): Promise<ReleaseDecision> {
    ...
    try {
      if(this.options.toolGateway&&token&&(exchange.upstreamStatus<200||exchange.upstreamStatus>=300))return{release:false,status:502,code:'upstream_terminal_invalid',message:'Upstream response cannot complete ATP run'};
